...
void MySetting::update_stng() {
	...
	thmTab->spc_pic->setPixmap(
			QPixmap(MyRes::shp_adds[SettingData::uShp]).scaled(thmTab->spc_pic->size(), Qt::KeepAspectRatio,
			                                                   Qt::SmoothTransformation));
	thmTab->env_pic->setPixmap(
			QPixmap(MyRes::env_adds[SettingData::env]).scaled(thmTab->env_pic->size(), Qt::KeepAspectRatio,
			                                                  Qt::SmoothTransformation));
	aiTab->aiShip_pic->setPixmap(
			QPixmap(MyRes::shp_adds[SettingData::aiShp]).scaled(aiTab->aiShip_pic->size(), Qt::KeepAspectRatio,
			                                                    Qt::SmoothTransformation));
	emit settingChanged();
}
